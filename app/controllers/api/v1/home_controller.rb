  ...
  module V1
    class HomeController < Api::V1::BaseController
      ...
      def cash_flows
        ...
        render json: { success: true, title: "Tahun #{params[:year]}", total_cash_in: total_cash_in, total_cash_out: total_cash_out, grand_total: grand_total, cash_flows: cash_flows.as_json(:methods => [:month_info]) }, status: :ok
      end
      ...
      def cash_transactions
        ...
        render json: { success: true, title: "#{UserContribution::MONTHNAMES.invert[params[:month].to_i]} #{params[:year]}", transactions: report_items, debit_total: debit_total, credit_total: credit_total, grand_total: debit_total-credit_total }, status: :ok
      end
      ...
      def notifications
        ...
        render status: :ok, json: {
          success: true,
          user_notifications: user_notifications.as_json(methods: [:notification, :notif_sent_at])
          }
      end
      ...
      def installments
        ...
        render status: :ok, json: {
          success: true,
          installments: installments.as_json(methods: [:total_paid, :remaining_installment, :paid_off?])
        }
      end
      ...
      def installment_transaction
        ...
        render status: :ok, json: {
          success: true,
          installment: installment.as_json(methods: [:total_paid, :remaining_installment, :paid_off?]),
          installment_transactions: installment_transactions
        }
      end

      def add_installment; end

      def pay_installment; end
