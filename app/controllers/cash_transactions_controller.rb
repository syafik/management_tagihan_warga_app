class CashTransactionsController < ApplicationController
  ...
  def index
    ...
    conditions =["year =  #{@year_selected}", "month = #{@month_selected}"] 
    conditions << "pic_id = #{params[:pic_id_eq]}" unless params[:pic_id_eq].blank? 
    conditions = conditions.join(" AND ")
    @cash_transactions = CashTransaction.includes(:pic).where(conditions).order('transaction_date ASC')
  ...
  def close_transaction
    if CashFlow.where(year: params[:year], month: params[:month]).exists?
      ...
      cash_transactions = CashTransaction.select('transaction_type, total, month, year').where( year: params[:year], month: params[:month])
      debit_total = cash_transactions.select{|t| t.transaction_type == CashTransaction::TYPE["DEBIT"]}.sum(&:total) 
      credit_total = cash_transactions.select{|t| t.transaction_type == CashTransaction::TYPE["KREDIT"]}.sum(&:total) 
      ...
      redirect_to cash_transactions_path, alert: "Transaction #{params[:month]}, #{params[:year]} was successfully closed."
    end
  end
  ...
  def show_report
    ...
    respond_to do |format|
      ...
      format.pdf {
        ...
        send_data(kit.to_pdf, :filename => "laporan_transaksi_#{params[:month]}_#{params[:year]}.pdf", :type => 'application/pdf', :disposition => 'inline')        
      }
    end
  end
