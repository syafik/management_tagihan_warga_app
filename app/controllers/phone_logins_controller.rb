class PhoneLoginsController < ApplicationController
  ...
  def authenticate
    ...
    if @user.login_code_valid?(login_code)
      ...
      if @user.login_code_expired?
        flash.now[:error] = 'Kode login telah kedaluwarsa. Silakan minta kode baru.'
      else
        flash.now[:error] = 'Kode login tidak valid. Silakan periksa dan coba lagi.'
      end
      render :verify
    end
  end
